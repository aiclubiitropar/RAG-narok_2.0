...
def get_llm(model_name: str = "rotate", temperature: float = 0.0, use_sum_key: bool = False, estimated_tokens: int = 0):
    ...
    if use_sum_key:
        model_name = random.choice(["gemma-2-27b-it", "gemma-2-9b-it", "qwen/qwen3-32b"])
        ...
        if estimated_tokens > 12000:
            ...
            models = ["llama-3.3-70b-versatile", "gemma-2-27b-it"] + gemini_models
            weights = [3, 3] + [1] * len(gemini_models)
            model_name = random.choices(models, weights=weights, k=1)[0]
        elif estimated_tokens > 3000:
            models = ["llama-3.3-70b-versatile", "gemma-2-27b-it", "gemma-2-9b-it", "openai/gpt-oss-120b", "openai/gpt-oss-20b"] + gemini_models
            weights = [3, 3, 3, 1, 1] + [1] * len(gemini_models)
            model_name = random.choices(models, weights=weights, k=1)[0]
        else:
            models = ["llama-3.3-70b-versatile", "gemma-2-27b-it", "gemma-2-9b-it", "qwen/qwen3-32b", "llama-3.1-8b-instant", "openai/gpt-oss-120b", "openai/gpt-oss-20b"] + gemini_models
